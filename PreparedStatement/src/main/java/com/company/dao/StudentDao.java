...
public class StudentDao {
    ...
    public Student fetchByEmail(String email) {
        ...
        con = ConnectionFactory.getConnection();
        try {
            // todo Найти студентов по Email
            String query = "select * from student where email_id=?";
            ps = con.prepareStatement(query);
            ps.setString(1, email);
            rs = ps.executeQuery();
            if (rs.next()) { // fixme if or while?
                student = getStudentFromResultSet(rs);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private Student getStudentFromResultSet(ResultSet rs) throws SQLException {
        ...
        student.setMail(rs.getString("email_id"));
    ...
    public Student fetchByMobileNo(String mobileNo) {
        ...
        con = ConnectionFactory.getConnection();
        try {
            // todo Найти студентов по Номеру телефона
    ...
    public List<Student> fetchBySalaryRange(BigDecimal lowerSalary, BigDecimal higherSalary) {
        ...
        con = ConnectionFactory.getConnection();
        try {
            // todo Найти студентов с ЗП в пределах От и До
    ...
    public List<Student> fetchByRangeDoj(Date startDate, Date endDate) {
        ...
        con = ConnectionFactory.getConnection();
        try {
            // todo Найти студентов с датами От и До
    ...
    public List<Student> fetchAll() {
        ...
        try {
            ...
            // todo Модернизируйте поиск - отсортировав от большего к меньшему по полю id
